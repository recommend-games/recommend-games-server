...
def percentile_buckets(values, percentiles):
    """Bucket boundaries at the given percentiles of a series of values.

    Vendored from board_game_recommender.utils (v3), which required Turi Create
    and does not exist in v4. Behaviour is preserved exactly, including that the
    lower bound stays at the series minimum for every bucket -- only the upper
    bound is consulted by star_rating().
    """
    ...
    assert percentiles == sorted(percentiles), "percentiles must be sorted"
    percentiles = (
        [p / 100 for p in percentiles] if max(percentiles) > 1 else percentiles
    )
    assert 0 < max(percentiles) < 1, "percentiles must be between 0 and 1"
    ...
    for percentile in percentiles:
        ...
        yield percentile, lower, values[index]


def star_rating(score, buckets, low=1, high=5):
    """Map a score onto a star rating using precomputed bucket boundaries.

    Vendored from board_game_recommender.utils (v3).
    """
    ...
    for index, (_, _, upper) in enumerate(buckets):
        if score <= upper:
